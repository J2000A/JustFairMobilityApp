...
public class DialogBuilder {
    ...
    public void showDatePickerDialog(Calendar datum, final Calendar minDate, final Calendar maxDate, final DialogBuilderDateSetter dateSetter){
        showDatePickerDialog(new DateCompat(datum), new DateCompat(minDate), new DateCompat(maxDate), dateSetter);
    }
    ...
    public void showDatePickerDialog(DateCompat datum, final DateCompat minDate, final DateCompat maxDate, final DialogBuilderDateSetter dateSetter){
        ...
        if (title.getText().toString().isEmpty())
            ...
            dialog.findViewById(R.id.top).setBackgroundColor(ContextCompat.getColor(context, R.color.colorSecondary));
            title.setTextColor(ContextCompat.getColor(context, R.color.colorOnSecondary));
            dialog.findViewById(R.id.close).setVisibility(View.GONE);
        }

        final DateCompat selected = (DateCompat) datum.clone();

        DatePicker datePicker = new DatePicker(context);
        datePicker.setLayoutParams(new ViewGroup.LayoutParams(WRAP_CONTENT,WRAP_CONTENT));
        datePicker.init(selected.getYear() + 1900, selected.getMonth(), selected.getDate(), (datePicker1, y, m, d) -> {
            selected.setYear(y-1900);
            selected.setMonth(m);
            selected.setDate(d);
        });
        ...
        setButtonLong(context.getString(R.string.ok), true, view -> dateSetter.dateChangedTo(selected.getTime()));
        ...
        dialog.findViewById(R.id.content).setPadding(0,0,0,0);

        title.setPadding(title.getPaddingLeft(),title.getPaddingTop(),title.getPaddingRight(), (int) context.getResources().getDimension(R.dimen.small));
    ...
    public void showZeitPickerDialog(Time time, final DialogBuilderDateSetter dateSetter){
        ...
        final DateCompat selected = new DateCompat();
